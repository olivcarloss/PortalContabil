...
- [x] Rewrite `NovoUsuarioModal` in `frontend/src/pages/licensing/UsuariosTab.tsx`: replace the UUID field with an e-mail field, make perfil de acesso required (remove the "nenhum perfil" option), call `licensingApi.convidarUsuario` instead of `createUsuario`+manual license loop — done when: manual test in the browser creates a user end-to-end via the new form with no UUID input visible. *(Code complete, type-checked, and the underlying API call verified live via curl (see task above). No browser-automation tool is available in this session to click through the UI myself — user should do a quick visual pass at http://localhost:5173 before merging.)*
- [x] Add a status badge column ("Convite pendente" / "Ativo") to the users table in `UsuariosTab.tsx` driven by `convite_status` — done when: badge renders correctly for a freshly invited (pending) and an already-active user in a manual browser check. *(convite_status confirmed correct via the live GET /licensing/usuarios call above ("ativo"); visual badge rendering not confirmed by me — same caveat as above.)*
